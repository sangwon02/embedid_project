...
def main():
    ...
    # 라운드 설정
    rounds_config = [
        {'duration': 1, 'spawn_sides': ['top', 'bottom'], 'spawn_rate': 0.4},
        {'duration': 1, 'spawn_sides': ['top', 'bottom', 'left', 'right'], 'spawn_rate': 0.4},
        {'duration': 1, 'spawn_sides': ['top', 'bottom', 'left', 'right'], 'spawn_rate': 0.3},
    ]
        ...
        while time.time() - round_start < config['duration']:
            ...
            for enemy in enemys[:]:
                ...
                if (abs(player_rect[0] - enemy_rect[0]) < 15 and  # 충돌 범위
                    ...
                    my_draw.text((joystick.width // 2 - 70, joystick.height // 2 - 16), "Game Over", font=font, fill=(255, 0, 0), stroke_width=2, stroke_fill=(0, 0, 0))
