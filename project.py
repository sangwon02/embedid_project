...
def main():
    ...
    start_draw.text((joystick.width // 2 - 70, joystick.height // 2 - 30), "TANK WAR", font=font, fill=(0, 0, 0))
    start_draw.text((joystick.width // 2 - 70, joystick.height // 2 - 30), "TANK WAR", font=font, fill=(255, 255, 255), stroke_width=2, stroke_fill=(0, 0, 0))
    start_draw.text((joystick.width // 2 - 105, joystick.height // 2 + 10), "Press button to start", font=font1, fill=(0, 0, 0))
    start_draw.text((joystick.width // 2 - 105, joystick.height // 2 + 10), "Press button to start", font=font1, fill=(255, 255, 255), stroke_width=2, stroke_fill=(0, 0, 0))
    joystick.disp.image(start_image)

    # 버튼 입력 대기
    while joystick.get_command() is None:  
        time.sleep(0.1)
    ...
    # 폭발 이미지 로드
    boom_image = Image.open("./image/effect_boom.png").resize((50, 50)) 
    ...
    for current_round, config in enumerate(rounds_config, start=1):
            ...
            if (not joystick.button_A.value or not joystick.button_B.value):
                if current_time - last_fire_time >= 0.5: 
                    if ammo_count > 0:
                        ...
                        ammo_message_start = time.time() 
                        while time.time() - ammo_message_start < 0.3:
                            command_list = joystick.get_command() 
                            player.move(command_list) 
                            ...
                            for bullet in bullets[:]:
                                ...
                                for enemy in enemys[:]:
                                    ...
                                    if (abs(bullet_rect[0] - enemy_rect[0]) < 18 and 
                                        abs(bullet_rect[1] - enemy_rect[1]) < 18):
                                        enemy.is_dead = True
                                        enemy.dead_time = time.time()
                                        bullets.remove(bullet)
                                        score += 1
                                        explosion_position = ((bullet_rect[0] + enemy_rect[0]) // 2, (bullet_rect[1] + enemy_rect[1]) // 2)
                                        explosions.append((explosion_position, time.time()))
                                        break
                            ...
                            for enemy in enemys[:]:
                                if enemy.is_dead:  
                                    continue
                                enemy_rect = enemy.get_rect()
                                if (abs(player_rect[0] - enemy_rect[0]) < 15 and 
                            ...
                            for explosion_position, explosion_time in explosions[:]:
                                ...
                                if elapsed_time <= 0.3:  
                                    my_image.paste(boom_image, tuple(np.array(explosion_position) - np.array([boom_image.width // 2, boom_image.height // 2])), boom_image)
                                else:
                                    explosions.remove((explosion_position, explosion_time))  
                            ...
                            for enemy in enemys:
                                ...
                                enemy_position = enemy.get_draw_position()
        ...
        if current_round < len(rounds_config):
            end_draw.text((joystick.width // 2 - 105, joystick.height // 2 - 16), f"Round {current_round + 1} START", font=font, fill=(0, 0, 0))  
            end_draw.text((joystick.width // 2 - 105, joystick.height // 2 - 16), f"Round {current_round + 1} START", font=font, fill=(255, 255, 255), stroke_width=2, stroke_fill=(0, 0, 0))  
            joystick.disp.image(end_image)
            time.sleep(1.5)
        else:  # 마지막 라운드면 게임 종료 메시지 출력
            end_draw.text((joystick.width // 2 - 90, joystick.height // 2 - 40), "Mission Complete!", font=font1, fill=(0, 0, 0))  
            ...
            end_draw.text((joystick.width // 2 - 50, joystick.height // 2 + 10), f"Score: {score}", font=font, fill=(0, 0, 0))
            end_draw.text((joystick.width // 2 - 50, joystick.height // 2 + 10), f"Score: {score}", font=font, fill=(255, 255, 255), stroke_width=2, stroke_fill=(0, 0, 0))  
            joystick.disp.image(end_image)
            time.sleep(1.5)
            break  # 게임 종료
